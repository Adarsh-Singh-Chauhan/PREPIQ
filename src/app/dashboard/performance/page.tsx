...
import { BarChart3, TrendingUp, TrendingDown, Award, Calendar, Download, Sparkles } from "lucide-react";
import {
...
const lineData = DEMO_SESSIONS.map((s) => ({
  date: new Date(s.date).toLocaleDateString("en-US", { month: "short", day: "numeric" }),
  score: s.overall_score,
}));

const domainData = [
  { domain: "Technical", score: 82 },
  { domain: "HR", score: 70 },
  { domain: "System Design", score: 78 },
  { domain: "Behavioural", score: 68 },
];



const best = DEMO_SESSIONS.reduce((a, b) => (a.overall_score > b.overall_score ? a : b));
const worst = DEMO_SESSIONS.reduce((a, b) => (a.overall_score < b.overall_score ? a : b));

// Generate heatmap data (12 weeks)
const heatmap: { day: number; week: number; level: number }[] = [];
for (let w = 0; w < 12; w++) {
  for (let d = 0; d < 7; d++) {
    heatmap.push({ week: w, day: d, level: ((w * 7 + d) * 13) % 5 });
  }
}
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card-static p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="badge badge-success text-xs"><TrendingUp size={10} />+13 pts</span>
        </div>
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={lineData}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
            <XAxis dataKey="date" tick={{ fill: "#A1A1AA", fontSize: 12 }} axisLine={false} tickLine={false} />
            <YAxis domain={[50, 100]} tick={{ fill: "#A1A1AA", fontSize: 12 }} axisLine={false} tickLine={false} />
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="card-static p-5">
          ...
          <p className="text-xs text-text-primary leading-relaxed">You improved 18% in Technical this week. Your HR score needs more attention — try 2 practice sessions.</p>
        </motion.div>
      </div>
    </div>
  );
